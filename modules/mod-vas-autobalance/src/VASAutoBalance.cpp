...
public:
    ...
    void ModifyCreatureAttributes(Creature* creature, bool resetSelLevel = false)
    {
        ...
        if (!creature->GetMap()->IsRaid() && !creature->GetMap()->IsBattleground() && DungeonsOnly == 1)
            return;
 	...
 	if(creatureVasInfo->instancePlayerCount < 5 && maxNumberOfPlayers == 10)
   	    creatureVasInfo->instancePlayerCount = 5;


	if(creatureVasInfo->instancePlayerCount <  15 && maxNumberOfPlayers == 25)
	   creatureVasInfo->instancePlayerCount = 15;
